### Change 1: Align Principal Attestation Schema With Rollout Semantics

**Rationale**: `PrincipalIdentityV1` currently makes `hardware_attestation_hash` mandatory while Stage S0 explicitly allows software-only bootstrap for `L0/L1`. This is an internal contradiction that can produce divergent implementations.

**Constraints preserved**: Preserves RFC-0020 fail-closed behavior, strict typing, and freshness policy (`INV-F-03`, `INV-F-08`, `INV-F-14`; `LAW-09`, `LAW-13`, `LAW-15`).

**Threat coverage**: Prevents accidental Tier3+ authorization with implicit/placeholder attestation data; closes downgrade ambiguity.

**Verification**: Add schema validation tests for nullable attestation fields and gate tests proving Tier3+ denies on `SOFTWARE_ONLY`.
...
### Change 2: Repair Epoch Chain And Challenge Binding Mismatch

**Rationale**: `verify_halt_liveness()` references epoch fields (`previous_seal_hash`, `challenge_nonce`) that do not exist in `SovereigntyEpochSealV1`. This makes replay protection underspecified.

**Constraints preserved**: Strengthens HTF authority, anti-replay, and fail-closed semantics from RFC-0020 (`INV-F-08`, `INV-F-11`, `INV-F-14`; `LAW-09`, `LAW-15`).

**Threat coverage**: Stops stale proof replay and epoch fork ambiguity in stop-liveness verification.

**Verification**: Add replay suites that mutate prior-epoch linkage and challenge nonces; require freeze on mismatch.
...
-6. `decoder_profile_hash` MUST resolve to active `SovereigntyDecoderProfileV1`; mismatch is containment uncertainty and triggers freeze.
...
+5. Tier2+ epochs MUST include non-null `principal_revocation_head_hash`, `hardware_kill_activation_receipt_hash`, `kill_switch_convergence_proof_hash`, and `capsule_integrity_proof_hash`.
...
### Change 3: Add Replay-Safe Stop-Order Binding To Hardware Kill Artifacts

**Rationale**: Hardware kill artifacts are not bound to a specific stop order + nonce. Old receipts can be replayed to fake fresh activation/convergence.

**Constraints preserved**: Preserves proof-carrying effects, digest-first bindings, and freshness (`INV-F-02`, `INV-F-08`, `INV-F-11`; `LAW-11`, `LAW-15`).

**Threat coverage**: Addresses replay/staleness attacks and forged convergence acceptance.

**Verification**: Add adversarial tests that reuse old `activation_id`/nonce across epochs; require gate failure.
...
+5. `stop_order_id` and `challenge_nonce_echo` MUST match the active `SovereignStopOrderV1`.
+6. Reuse of `(stop_order_id, challenge_nonce_echo)` across distinct `activation_id` values is a replay defect and MUST fail closed.
@@
...
### Change 4: Make Tier3+ Liveness Verification Explicitly Dual-Verifier And Complete

**Rationale**: Current liveness algorithm does not require proof-seal verification, multiproof checking, or mandatory dual independence at Tier3+.

**Constraints preserved**: Reinforces containment-over-liveness ordering and measurable stop integrity (`INV-F-11`, `INV-F-14`, `INV-F-15`; `LAW-01`, `LAW-08`, `LAW-15`).

**Threat coverage**: Covers verifier capture, forged PASS verdicts, and partial-proof acceptance.

**Verification**: Add gate predicates for independent verifier count and mutation tests for invalid multiproof/seal.
...
-  require proof.prior_epoch_seal_hash == epoch.previous_seal_hash when epoch is non-genesis
...
+  required_verifiers = (scope.max_risk_tier >= Tier3) ? 2 : 1
...
+| GATE-PSI-HALT-PATH | T | S0 | `propagation_p99_ms<=2000 && deny_uncertainty_p99_ms<=250 && tier3plus_independent_pass_verifier_count>=2` | Any threshold breach or insufficient independent verifier coverage | `HaltPathLivenessProofV1` |
@@
...
+| GATE-PSI-HALT-PATH | Security Council + Runtime Council | `HTF-BND-P1-SECURITY-CLOSE` | `jq -e '.propagation_p99_ms <= 2000 and .deny_uncertainty_p99_ms <= 250 and .tier3plus_independent_pass_verifier_count >= 2 and .unknown_state_count == 0' evidence/rfcs/RFC-0022/gates/GATE-PSI-HALT-PATH/summary.json` | `evidence/rfcs/RFC-0022/gates/GATE-PSI-HALT-PATH/` |
```

---

### Change 5: Formalize Audit Lattice Semantics And Non-Mediated Bypass Contract

**Rationale**: Confidentiality/integrity ordering is currently ambiguous (“dominates”) and audit bypass is only implied by a manifest hash without explicit independence contract.

**Constraints preserved**: Strengthens dual-lattice governance from RFC-0020 (`INV-F-05`, `INV-F-12`, `INV-F-13`, `INV-F-14`; `LAW-05`, `LAW-07`, `LAW-15`).

**Threat coverage**: Addresses mediated-view censorship, label-order confusion, and classification bypass.

**Verification**: Add lattice conformance tests and gate predicates for `audit_bypass_manifest_valid`.
...
+5. `served_integrity_floor` and `served_confidentiality_ceiling` MUST satisfy Section 3.3a lattice-order rules.
+6. If `request.include_confidential_payloads=false`, response MUST keep `served_confidentiality_ceiling=PUBLIC` and `declassification_receipt_hashes=[]`.
+7. `direct_fetch_manifest_hash` MUST provide digest-addressable retrieval endpoints independent of responder projections.
+8. `audit_bypass_manifest_hash` MUST resolve to valid `AuditBypassManifestV1`.
+9. Response without completeness attestation is advisory only and non-authoritative.
+
+### 4.11a AuditBypassManifestV1
+```yaml
...
### Change 6: Bind Containment Proofs To Active Runtime Surface (Not Just Declared Surface)

**Rationale**: `ContainmentProofBundleV1` proves coverage claims but does not currently require binding to active runtime measurements at a specific epoch.

**Constraints preserved**: Strengthens containment dominance and measurement integrity (`INV-F-14`, `INV-F-15`; `LAW-05`, `LAW-15`).

**Threat coverage**: Prevents stale/off-surface containment proofs being reused after runtime/policy drift.

**Verification**: Add `GATE-PSI-SOVEREIGNTY-SURFACE-BINDING` and require multiproof verification against active measurements.
...
@@
-| S5 | `HTF-BND-P2-SEMANTICS-CLOSE` | `HTF-BND-P3-CLOSE` | `GATE-PSI-CONTAINMENT-PROOF`, `GATE-PSI-VERIFIER-INDEPENDENCE`, `GATE-PSI-VERIFIER-DISAGREEMENT` | Internal-only containment proof, verifier capture, or verifier disagreement |
...
### Change 7: Resolve Complexity Contract Inconsistency

**Rationale**: Section 3.6 states `O(k log b)` while acceptance criteria and audit semantics require `O(log n + k)` for batch proofs.

**Constraints preserved**: Keeps proof-economics commitments aligned with RFC-0020 scale model (`LAW-08`, `INV-F-10`, `INV-F-15`).

**Threat coverage**: Prevents unverifiable complexity claims and Goodhart drift on audit proof performance.

**Verification**: Require `proof_profile_valid` in acceptance summary.
...
### Change 8: Mechanize Self-Modification Lock For Sovereignty Constraints

**Rationale**: OBJ-PSI-06 requires proof the system cannot alter its own sovereignty constraints. Current draft has containment proofs but no explicit mutation-admission protocol object or gate.

**Constraints preserved**: Strengthens containment-first dominance and external-verifier requirement (`INV-F-14`, `INV-F-15`; `LAW-01`, `LAW-05`, `LAW-08`, `LAW-15`).

**Threat coverage**: Adds explicit control against runtime self-mutation of sovereignty roots.

**Verification**: Introduce `GATE-PSI-SELF-MOD-LOCK`, mutation chain checks, and adversarial unauthorized-mutation tests.
...
+  require mutation.prior_constraint_root_hash == active_constraint_root_hash()
+  require mutation.effective_epoch_id >= current_epoch_id()
...
+| GATE-PSI-SELF-MOD-LOCK | T | S0 | `unauthorized_sovereignty_constraint_mutation_count == 0 && mutation_receipt_chain_valid == true` | Any unreceipted sovereignty constraint mutation | `SovereigntyConstraintMutationReceiptV1` |
@@
...
+| GATE-PSI-SELF-MOD-LOCK | Security Council + External Assurance | `HTF-BND-P3-CLOSE` | `jq -e '.unauthorized_sovereignty_constraint_mutation_count == 0 and .mutation_receipt_chain_valid == true and .unknown_state_count == 0' evidence/rfcs/RFC-0022/gates/GATE-PSI-SELF-MOD-LOCK/summary.json` | `evidence/rfcs/RFC-0022/gates/GATE-PSI-SELF-MOD-LOCK/` |
@@
...
### Change 9: Normalize Ticket Dependencies To Canonical IDs

**Rationale**: Mixed shorthand dependencies (`0002..0011`, `0001`) are ambiguous for machine parsing and tooling.

**Constraints preserved**: Preserves deterministic decomposition and replayable planning artifacts (`LAW-13`, `INV-F-02`).

**Threat coverage**: Prevents dependency mis-resolution and accidental out-of-order implementation.

**Verification**: Validate with ticket emitter/parser and CI lint for dependency resolution.
...
+| `TCK-PSI-0011` | `TCK-PSI-0004`,`TCK-PSI-0005`,`TCK-PSI-0006` | Kernel event wire integration for tags 35..45 | `event_replay_equivalence_ratio == 1.0` | `evidence/rfcs/RFC-0022/tickets/TCK-PSI-0011/` |
...
Remaining gaps after these patches:
1. Tooling-level publication completeness is still outside this Markdown: if your release pipeline expects companion RFC artifacts (for example trust-boundary YAMLs), those still need to be generated.
2. `proto/kernel_events.proto` and schema registries must be updated in lockstep with Section 5/Section 4 additions; otherwise wire-level claims remain aspirational.
