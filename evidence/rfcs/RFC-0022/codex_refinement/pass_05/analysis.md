### Change 1: Make Dominance Evidence Explicitly Strategy-Aligned

**Rationale**: The draft has humanitarian admissibility text, but the machine predicate only checks the 3-layer dominance order. That leaves a governance ambiguity relative to strategy dominance (`humanitarian_alignment` first). Add explicit machine-checkable humanitarian/containment precheck fields.

**Constraints preserved**: INV-F-14 precedence, LAW-05 containment, strategy dominance order.

**Threat coverage**: Governance downgrade and “principal seal bypasses humanitarian deny” class.

**Verification**: Updated `PSI-GOV-ORDER-001` predicate asserts both prechecks.
...
### Change 2: Fix Autonomy Ceiling Composition and Replay-Key Semantics

**Rationale**: `L_eff` currently omits principal ceiling, and replay rule `(issuer_key_id, sovereignty_epoch)` is too coarse (blocks legitimate multiple commands per epoch). Replay key must include nonce and scope/subject binding.

**Constraints preserved**: LAW-14 proportionality, INV-F-05 least privilege, LAW-09 freshness.

**Threat coverage**: Capability escalation via config drift; false replay rejects; replay acceptance across scopes.

**Verification**: Add multi-command-same-epoch tests with unique nonce/scope; replay-negative tests for duplicate tuple.
...
### Change 3: Define Missing Referenced Schemas (Classification + Proof Complexity)

**Rationale**: `classification_lattice_profile_hash` and `proof_profile_hash` are referenced but their schemas are absent. This blocks machine-checkable enforcement.

**Constraints preserved**: RFC-0020 typed/canonical contracts, INV-F-03 typed boundaries, LAW-13 semantic contracting.

**Threat coverage**: Label-order bypass, unverifiable complexity claims, implementation divergence.

**Verification**: Schema registry tests and profile-signature validation tests.
...
### Change 4: Anchor Active Constraint Root and Audit Bypass Root in Epoch Seals

**Rationale**: Self-modification control currently lacks epoch-level binding of the active sovereignty constraint root. Audit bypass endpoint integrity is also not anchored at epoch level.

**Constraints preserved**: OBJ-PSI-06, INV-F-14, LAW-15 measurement integrity, LAW-01 loop closure.

**Threat coverage**: Undetected sovereignty-root mutation, mediated audit endpoint substitution.

**Verification**: Gate predicates require active root binding and epoch-anchored bypass manifest checks.
...
### Change 5: Make Hardware Kill Truly Software-Independent

**Rationale**: Current hardware activation receipt requires active stop-order binding. That weakens OBJ-PSI-01 under total software compromise. Physical activation must remain authoritative even if software control path is unavailable.

**Constraints preserved**: OBJ-PSI-01, INV-F-11 fail-closed stop checks, LAW-05 dual-axis containment.

**Threat coverage**: Software-subverted stop pipeline blocking principal kill action.

**Verification**: Drill with software sovereignty kernel intentionally down; hardware path must still produce valid activation and convergence evidence.
...
### Change 6: Add Explicit Sovereign Resume Protocol (No Implicit Unfreeze)

**Rationale**: Freeze semantics are strong, but there is no typed sovereign resume artifact. Relying on expiry or legacy unfreeze paths is unsafe and under-specified.

**Constraints preserved**: INV-F-11/14, LAW-01 loop closure, LAW-15 measurement integrity.

**Threat coverage**: Accidental or malicious implicit unfreeze; stale-state resume.

**Verification**: Negative tests that expiry and `InterventionUnfreeze` cannot clear sovereign freeze; resume requires fresh liveness/containment/revocation proofs.
...
### Change 7: Harden Liveness-Probe Semantics and Fix Algorithm Bugs

**Rationale**: Current liveness algorithm uses undefined `scope` in verifier count and per-proof `nonce_unused` check that can reject valid dual-verifier proofs in the same epoch. Probe anti-gaming coverage/randomness is not explicit.

**Constraints preserved**: OBJ-PSI-02, LAW-08 Goodhart resistance, INV-F-15 verifier integrity.

**Threat coverage**: Probe gaming false PASS; replay confusion; verifier disagreement edge-case false negatives.

**Verification**: Dual-verifier same-epoch tests, randomized probe-plan determinism tests, minimum sample-coverage checks.
...
### Change 8: Close Governance/Acceptance/Ticket Loops for New Mechanics

**Rationale**: After adding/strengthening protocol mechanics, gate predicates, acceptance bundle, and tickets must be updated or the spec stays partially non-mechanized.

**Constraints preserved**: LAW-01 loop closure, INV-F-02 receipt-bound promotion, INV-F-15 machine-verifiable acceptance.

**Threat coverage**: “Spec says X, gates don’t enforce X” drift.

**Verification**: Gate registry and acceptance predicates include the new required signals; ticket plan covers implementation.
...
Remaining gaps after these diffs are mostly parameterization, not architecture: concrete policy values for `min_probe_coverage_ratio`, maximum allowed drill staleness window, and concrete cadence for independent-verifier rotation should still be fixed in policy artifacts before release.