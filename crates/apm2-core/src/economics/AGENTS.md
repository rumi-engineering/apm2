...
- `StopRevokeAdmissionTrace` -- Structured admission trace bound to receipts: verdict, reservation/carve-out activation state, temporal predicate outcomes, queue backlog snapshot, and policy snapshot. `canonical_bytes()` returns deterministic canonical JSON bytes for replay verification.
