...
/// intent field are accepted for backwards compatibility with pre-TCK-00567
/// tokens; the worker logs a warning but does not deny.
///
/// TCK-00566: Added `nonce` field for single-use replay protection.
/// The broker records each nonce in its token-use ledger at issuance time;
/// a second use of the same nonce is denied.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct TokenBindingV1 {
    ...
    pub expiry_tick: u64,
    /// RFC-0028 typed intent class bound to this token (TCK-00567).
    ///
    /// When `Some`, the worker MUST verify that this intent matches the
    /// expected intent for the job kind.  `None` for pre-TCK-00567 tokens.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub intent: Option<String>,
    /// TCK-00566: Single-use nonce for replay protection.
    ///
    /// Generated by the broker at issuance time and recorded in the
    ...
    ///
    /// Defaults to `None` for backwards compatibility with pre-TCK-00566
...
pub enum ChannelContextTokenError {
    ...
    /// Token intent does not match the expected intent (TCK-00567).
    ///
    /// The broker-issued token was bound to a different intent than the one
    /// required by the job kind.  Fail-closed: a token for GC cannot be
    /// used to execute gates.
    #[error("token binding: intent mismatch: expected {expected}, got {actual}")]
    IntentMismatch {
        /// Expected intent string.
        expected: String,
        /// Actual intent string carried in the token.
        actual: String,
    },

    /// Token is missing an intent field when the worker expected one
    /// (TCK-00567).
    #[error("token binding: missing intent field (expected {expected})")]
    MissingIntent {
        /// Expected intent string.
        expected: String,
    },
...
pub struct ExpectedTokenBinding<'a> {
    ...
    /// (TCK-00567).  When `Some`, the token's `intent` field MUST match
    /// exactly.  When `None`, intent validation is skipped (pre-TCK-00567
    /// compatibility).
...
pub fn decode_channel_context_token_with_binding(
    ...
    if let Some(expected) = expected_binding {
        ...
        // TCK-00567: Validate intent binding (fail-closed).
        if let Some(expected_intent) = expected.expected_intent {
            match &binding.intent {
                Some(token_intent) if token_intent == expected_intent => {
                    // Match — intent is authorized.
                },
                Some(token_intent) => {
                    return Err(ChannelContextTokenError::IntentMismatch {
                        expected: expected_intent.to_string(),
                        actual: token_intent.clone(),
                    });
                },
                None => {
                    return Err(ChannelContextTokenError::MissingIntent {
                        expected: expected_intent.to_string(),
                    });
                },
            }
...
mod tests {
    ...
    // -----------------------------------------------------------------------
    // TCK-00567: Intent taxonomy — kind/intent mismatch denial
    // -----------------------------------------------------------------------

    #[test]
    fn tck_00567_gc_token_cannot_execute_gates() {
        // DoD: "A token for GC cannot be used to execute gates
        // (kind/intent mismatch denial)".
        let signer = Signer::generate();
        let mut binding = default_binding();
        binding.intent = Some("intent.fac.gc".to_string());
        let (token, _check) = make_token_with_binding(&signer, binding);

        // Worker expects gates intent — GC token MUST be denied.
        let expected = ExpectedTokenBinding {
            fac_policy_hash: &[0xAA; 32],
            canonicalizer_tuple_digest: &[0xBB; 32],
            boundary_id: "local",
            current_tick: 150,
            expected_intent: Some("intent.fac.execute_gates"),
        };
        ...
        assert!(
            matches!(result, Err(ChannelContextTokenError::IntentMismatch { .. })),
            "GC token must be denied when gates intent is expected, got: {result:?}"
        );
    }

    #[test]
    fn tck_00567_intent_match_allows_decode() {
        // A token whose intent matches the worker expectation must decode
        // successfully (happy path).
        let signer = Signer::generate();
        let mut binding = default_binding();
        binding.intent = Some("intent.fac.execute_gates".to_string());
        let (token, _check) = make_token_with_binding(&signer, binding);
        ...
        assert!(
            result.is_ok(),
            "matching intent must allow decode, got: {result:?}"
        );
    }

    #[test]
    fn tck_00567_missing_intent_denied_when_expected() {
        // A token with no intent field MUST be denied when the worker
        // expects a specific intent (fail-closed).
        let signer = Signer::generate();
        let binding = default_binding(); // intent: None
        let (token, _check) = make_token_with_binding(&signer, binding);
        ...
        assert!(
            matches!(result, Err(ChannelContextTokenError::MissingIntent { .. })),
            "token without intent must be denied when intent is expected, got: {result:?}"
        );
    }

    #[test]
    fn tck_00567_no_expected_intent_skips_intent_check() {
        // When the worker does not set expected_intent, any token intent
        // (or lack thereof) is accepted — backwards compat.
        let signer = Signer::generate();
        let mut binding = default_binding();
        binding.intent = Some("intent.fac.gc".to_string());
        let (token, _check) = make_token_with_binding(&signer, binding);
        ...
        let expected = ExpectedTokenBinding {
            ...
            boundary_id: "local",
            current_tick: 150,
            expected_intent: None, // no intent expectation
        };
        ...
        assert!(
            result.is_ok(),
            "no expected intent must skip intent check, got: {result:?}"
        );
    }

    #[test]
    fn tck_00567_warm_token_cannot_execute_gates() {
        // Cross-kind mismatch: warm intent denied for gates.
        let signer = Signer::generate();
        let mut binding = default_binding();
        binding.intent = Some("intent.fac.warm".to_string());
        let (token, _check) = make_token_with_binding(&signer, binding);
        ...
        assert!(
            matches!(result, Err(ChannelContextTokenError::IntentMismatch { .. })),
            "warm token must be denied for gates, got: {result:?}"
        );
    }
}
