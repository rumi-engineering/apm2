...
async fn test_allow_decision_with_matching_capability() {
    ...
    let decision = broker.request(&request, current_timestamp_ns()).await;
...
async fn test_allow_multiple_tool_classes() {
    ...
    let decision = broker
        .request(&read_request, current_timestamp_ns())
        .await
        .unwrap();
    ...
    let decision = broker
        .request(&write_request, current_timestamp_ns())
        .await
        .unwrap();
...
async fn test_deny_no_matching_capability() {
    ...
    let decision = broker
        .request(&request, current_timestamp_ns())
        .await
        .unwrap();
...
async fn test_dedupe_cache_hit() {
    ...
    let decision1 = broker.request(&request, timestamp).await.unwrap();
    ...
    let decision2 = broker.request(&request2, timestamp + 1000).await.unwrap();
...
async fn test_dedupe_cache_miss_different_key() {
    ...
    let decision1 = broker.request(&request1, timestamp).await.unwrap();
    assert!(decision1.is_allowed());
