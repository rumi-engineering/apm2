...
///
/// Per RFC-0032::REQ-0070, the `tool_allowlist` is automatically populated from the
...
async fn test_allow_decision_with_matching_capability() {
    ...
    // validation, not policy engine integration (RFC-0032::REQ-0094 default-deny behavior)
...
async fn test_allow_multiple_tool_classes() {
    ...
    // the shell bridge allowlist check (the default is deny_all per RFC-0020::REQ-0031).
    let shell_policy =
...
async fn test_dedupe_cache_hit() {
    ...
    // behavior, not policy engine integration (RFC-0032::REQ-0094 default-deny behavior)
    let broker: TestToolBroker =
        ToolBroker::new(ToolBrokerConfig::default().without_policy_check());
    broker
        .initialize_with_manifest(manifest)
...
async fn test_dedupe_cache_episode_isolation() {
    ...
    let manifest = create_manifest_with_capabilities(vec![read_capability("/workspace")]);
    // NOTE: Use without_policy_check() because this test focuses on dedupe cache
    // episode isolation, not policy engine integration (RFC-0032::REQ-0094 default-deny
    // behavior)
    let broker: TestToolBroker =
        ToolBroker::new(ToolBrokerConfig::default().without_policy_check());
    broker
...
async fn test_broker_with_mock_harness() {
    ...
    // integration, not policy engine integration (RFC-0032::REQ-0094 default-deny behavior)
    let broker: TestToolBroker =
        ToolBroker::new(ToolBrokerConfig::default().without_policy_check());
    broker
        .initialize_with_manifest(manifest)
