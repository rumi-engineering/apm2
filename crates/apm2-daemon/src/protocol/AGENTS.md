...
- TCK-00636: `ResolveTicketAlias` and `SpawnEpisode` alias handling (RFC-0032 Phase 1). **Invariant (Canonical-ID Bypass)**: `SpawnEpisode` only calls `resolve_ticket_alias()` for alias-like IDs (currently `TCK-*` inputs). Canonical `work_id` inputs bypass alias-index lookup entirely so alias-index lossy-marker saturation cannot deny all spawns. **Invariant (Alias Fail-Closed)**: alias-like inputs still fail closed on ambiguous/lossy/saturated alias resolution.
