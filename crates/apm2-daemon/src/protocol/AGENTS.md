...
- TCK-00635: `OpenWork` (IPC-PRIV-076) handler persists WorkSpec to CAS and emits a canonical `work.opened` event. Idempotent on `(work_id, spec_hash)` via bounded `LIMIT 1` query (not full event scan): same hash returns success with `already_existed=true`; different hash returns `WORK_ALREADY_EXISTS`. Actor ID derived from peer credentials (never client input). CAS must be configured (fail-closed). PCAC lifecycle enforcement is required before CAS/ledger mutation (fail-closed when PCAC gate is not wired). `lease_id` is required for PCAC revalidation inputs. Race-safe: a partial unique index `idx_work_opened_unique ON ledger_events(work_id) WHERE event_type = 'work.opened'` enforces at-most-one `work.opened` per `work_id` at the database level; UNIQUE constraint violations are caught in the handler, the persisted event is re-read, and idempotent success or `WORK_ALREADY_EXISTS` is returned. The in-memory stub enforces the same invariant within its write lock.
