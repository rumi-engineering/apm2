...
- TCK-00636: `ResolveTicketAlias` and `SpawnEpisode` alias handling (RFC-0032 Phase 1). **Invariant (Canonical-ID Bypass)**: `SpawnEpisode` only calls `resolve_ticket_alias()` for alias-like IDs (currently `TCK-*` inputs). Canonical `work_id` inputs bypass alias-index lookup entirely so alias-index lossy-marker saturation cannot deny all spawns. **Invariant (Alias Fail-Closed)**: alias-like inputs still fail closed on ambiguous/lossy/saturated alias resolution. **Invariant (Shared Projection State)**: dispatcher wiring reuses a single `WorkObjectProjection` handle for both `ProjectionWorkAuthority` and `ProjectionAliasReconciliationGate` to avoid divergent in-memory authority snapshots.
...
- TCK-00645: RFC-0032 Phase 4 PublishWorkLoopProfile handler (IPC-PRIV-079) -- bounded+validated profile decode (`<=64KiB`, deny unknown fields), deterministic `WLP-` evidence ID from `(work_id, dedupe_key)`, CAS-first storage via `publish_work_loop_profile`, and `evidence.published` emission with category `WORK_LOOP_PROFILE`. **Invariant (Lease-Bound Admission)**: request MUST include `lease_id`; authorization resolves claims via `get_claim_by_lease_id(work_id, lease_id)` and enforces actor+lease constant-time binding before mutation. **Invariant (PCAC Before Idempotency)**: privileged lifecycle (`derive_privileged_pcac_revalidation_inputs` + `enforce_privileged_pcac_lifecycle` join/revalidate/consume) executes before replay lookup and before CAS/ledger effects. **Invariant (Evidence Sufficiency)**: emitted `evidence.published.metadata` includes PCAC selectors (`pcac_ajc_id`, `pcac_intent_digest`, `pcac_consume_tick`, `pcac_time_envelope_ref`, `pcac_consume_selector_digest`) for audit parity with other privileged mutation handlers. **Invariant (Idempotent on `(work_id, dedupe_key)`)**: duplicate requests return the originally anchored CAS hash and do not emit duplicate ledger events.
...
- TCK-00669: `LedgerEventEmitter::get_events_since(cursor_timestamp_ns, cursor_event_id, event_types, limit)` is the bounded cursor API for projection replay. Projection/reconciler code must use this method (with explicit event-type filters) instead of `get_all_events()` to avoid per-tick O(N) history materialization.
- TCK-00640 (2026-02-24 update): Push/work identity hardening invariants are now explicit in code labels. Preserve `INV-FAC-WORK-BIND-001` (authoritative lease->work binding before privileged mutation), `INV-FAC-CLAIM-BIND-001` (lease-scoped claim resolution only), `INV-FAC-ACTOR-BIND-001` (constant-time actor ownership check), and `INV-FAC-PR-BIND-001` (`WORK_PR_BINDING_CONFLICT` anti-flap rejection on conflicting work->PR binding).
