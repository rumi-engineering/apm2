...
- TCK-00637: RFC-0032 Phase 2 ClaimWorkV2 handler (IPC-PRIV-078) -- claims existing work_id, issues leases, anchors authority bindings via CAS. **Invariant (Lease+Claim Registration)**: The handler registers the issued lease in `LeaseValidator` (via `register_lease_with_executor`) and the claim in `WorkRegistry` (via `register_claim`) so that downstream handlers (`SpawnEpisode`, `DelegateSublease`, etc.) can resolve lease-to-work mappings and claim authority. **Invariant (Event Encoding)**: Uses legacy `emit_work_transitioned` (event_type `work_transitioned`) with JSON payload instead of `emit_session_event` with `work.transitioned`, because the projection bridge (`translate_signed_events`) normalizes `work_transitioned` JSON payloads into protobuf but passes `work.` events through as-is (expecting native protobuf). **Invariant (Bounded Idempotency)**: Idempotency checks use bounded `get_first_event_by_work_id_and_type` (LIMIT 1) and proper JSON deserialization (not raw byte window matching) for rationale code and evidence category verification. **Invariant (Actual Policy Hashes)**: `WorkAuthorityBindingsV1` records actual policy artifact hashes (`capability_manifest_hash`, `context_pack_hash`, `resolved_policy_hash`) from the resolved `PolicyResolution`, not synthetic PCAC-derived hashes. **Invariant (Risk Tier)**: PCAC risk tier is derived from `resolve_risk_tier_for_lease` using the governing lease's policy resolution (fail-closed to Tier4 when lease/claim not found).
