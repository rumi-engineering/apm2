...
- TCK-00637: RFC-0032 Phase 2 ClaimWorkV2 handler (IPC-PRIV-078) -- claims existing work_id, issues leases, anchors authority bindings via CAS. **Invariant (Actor-Lease Binding)**: The handler verifies that `ctx.actor_id()` matches the `executor_actor_id` bound to the governing `lease_id` using constant-time comparison (`subtle::ConstantTimeEq`). This prevents authorization bypass where Actor B reuses a lease_id belonging to Actor A (lease_ids are not secret — they appear in audit logs and ledger events). **Invariant (Lease+Claim Registration)**: The handler registers the issued lease in `LeaseValidator` (via `register_lease_with_executor`) and the claim in `WorkRegistry` (via `register_claim`) so that downstream handlers (`SpawnEpisode`, `DelegateSublease`, etc.) can resolve lease-to-work mappings and claim authority. **Invariant (Role-Indexed Claims)**: `WorkRegistry` is keyed by `(work_id, role)` to support multi-role workflows where Implementer and Reviewer each claim the same `work_id`. `get_claim_for_role(work_id, role)` returns role-specific claims; `get_claim(work_id)` returns the first registered claim for backward compatibility. **Invariant (Event Encoding)**: Uses legacy `emit_work_transitioned` (event_type `work_transitioned`) with JSON payload instead of `emit_session_event` with `work.transitioned`, because the projection bridge (`translate_signed_events`) normalizes `work_transitioned` JSON payloads into protobuf but passes `work.` events through as-is (expecting native protobuf). **Invariant (Role-Filtered Idempotency)**: Idempotency checks search all `work_transitioned` events in reverse order (newest first) and filter by the requested role's `rationale_code` and `actor_id`, ensuring Reviewer re-claims do not collide with Implementer claims. If claim details are irrecoverable, the handler returns an error (fail-closed) instead of a hollow success with empty fields. **Invariant (Actual Policy Hashes)**: `WorkAuthorityBindingsV1` records actual policy artifact hashes (`capability_manifest_hash`, `context_pack_hash`, `resolved_policy_hash`) from the resolved `PolicyResolution`, not synthetic PCAC-derived hashes. The handler fails closed if no actual policy resolution is available. **Invariant (Risk Tier)**: PCAC risk tier is derived from `resolve_risk_tier_for_lease` using the governing lease's policy resolution (fail-closed to Tier4 when lease/claim not found).
