...
- TCK-00635: `OpenWork` (IPC-PRIV-076) handler persists WorkSpec to CAS and emits a canonical `work.opened` event. Idempotent on `(work_id, spec_hash)`: same hash returns success with `already_existed=true`; different hash returns `WORK_ALREADY_EXISTS`. Actor ID derived from peer credentials (never client input). CAS must be configured (fail-closed).
