...
pub struct PeerCredentials {
    ...
    pub gid: u32,
    /// Process ID of the peer process (optional on some platforms, but usually
    /// available on Linux).
    pub pid: Option<i32>,
}
...
mod tests {
    ...
    #[tokio::test]
    #[allow(clippy::similar_names)] // uid and gid are standard system terminology
    async fn test_peer_credentials_extraction() {
        ...
        let expected_gid = getgid().as_raw();

        assert_eq!(creds.uid, expected_uid);
        assert_eq!(creds.gid, expected_gid);
        // PID might vary if extracted from kernel perspective but usually current PID
        // or None for pair? Actually socketpair doesn't always populate
        // credentials correctly on all platforms or versions but typically it
        // does. We'll assert it's present if we expect it.
