...
///
/// Real work is performed inside `step` handlers for each state. The machine
/// carries the minimal durable artifacts needed to preserve crash/reconcile
/// semantics (claimed lock, lane lock, claimed path, and phase traces).
...
pub(super) enum StepOutcome {
    /// Orchestrator advanced to a new state; call `step` again.
    Advanced,
...
enum IdlePhaseOutcome {
    Advanced(Box<ClaimedContext>),
    Done(JobOutcome),
}

enum ClaimedAcquireOutcome {
    Advanced(Box<LaneAcquiredContext>),
    Done(JobOutcome),
}

enum LanePreparedOutcome {
    Advanced(Box<LeasePersistedContext>),
    Done(JobOutcome),
}
...
struct ClaimedContext {
    ...
    sbx_hash: String,
    resolved_net_hash: String,
}

struct LaneAcquiredContext {
    claimed: ClaimedContext,
    lane_mgr: LaneManager,
    lane_guard: LaneLockGuard,
    lane_id: String,
}

struct LeasePersistedContext {
    acquired: LaneAcquiredContext,
    workspace: PathBuf,
    dir: PathBuf,
    profile_hash: String,
    systemd_properties: SystemdUnitProperties,
}

pub(super) struct OrchestratorContext<'a> {
    pub(super) candidate: &'a PendingCandidate,
    pub(super) queue_root: &'a Path,
    pub(super) fac_root: &'a Path,
    pub(super) completed_gates_cache: &'a mut Option<CompletedGatesCache>,
    pub(super) verifying_key: &'a apm2_core::crypto::VerifyingKey,
    pub(super) scheduler: &'a QueueSchedulerState,
    pub(super) lane: QueueLane,
    pub(super) broker: &'a mut FacBroker,
    pub(super) signer: &'a Signer,
    pub(super) policy_hash: &'a str,
    pub(super) policy_digest: &'a [u8; 32],
    pub(super) policy: &'a FacPolicyV1,
    pub(super) job_spec_policy: &'a apm2_core::fac::JobSpecValidationPolicy,
    pub(super) budget_cas: &'a MemoryCas,
    pub(super) print_unit: bool,
    pub(super) canonicalizer_tuple_digest: &'a str,
    pub(super) boundary_id: &'a str,
    pub(super) heartbeat_cycle_count: u64,
    pub(super) heartbeat_jobs_completed: u64,
    pub(super) heartbeat_jobs_denied: u64,
    pub(super) heartbeat_jobs_quarantined: u64,
    pub(super) cost_model: &'a apm2_core::economics::CostModelV1,
    pub(super) toolchain_fingerprint: Option<&'a str>,
}

pub(super) struct WorkerOrchestrator {
    state: OrchestratorState,
    terminal_outcome: Option<JobOutcome>,
    claimed: Option<ClaimedContext>,
    lane_acquired: Option<LaneAcquiredContext>,
    lease_persisted: Option<LeasePersistedContext>,
...
impl WorkerOrchestrator {
    ...
    fn clear_phase_data(&mut self) {
        self.claimed = None;
        self.lane_acquired = None;
        self.lease_persisted = None;
        self.staged_outcome = None;
    }
    ...
    pub(super) fn step(&mut self, ctx: &mut OrchestratorContext<'_>) -> StepOutcome {
        ...
        match self.state.clone() {
            ...
            OrchestratorState::Claimed { .. } => {
                let Some(claimed) = self.claimed.take() else {
                    let outcome = JobOutcome::skipped(
                        "orchestrator missing claimed context while in Claimed state",
                    );
                    self.complete_with_outcome(ctx.candidate.spec.job_id.clone(), outcome.clone());
                    return StepOutcome::Done(outcome);
                };
                match Self::run_claimed_acquire_phase(ctx, claimed) {
                    ClaimedAcquireOutcome::Done(outcome) => {
                        self.complete_with_outcome(
                            ctx.candidate.spec.job_id.clone(),
                            outcome.clone(),
                        );
                        StepOutcome::Done(outcome)
                    },
                    ClaimedAcquireOutcome::Advanced(lane_acquired) => {
                        let lane_acquired = *lane_acquired;
                        let lane_id = lane_acquired.lane_id.clone();
                        self.lane_acquired = Some(lane_acquired);
                        self.transition(OrchestratorState::LaneAcquired {
                            job_id: ctx.candidate.spec.job_id.clone(),
                            lane_id,
                        });
                        StepOutcome::Advanced
            ...
            OrchestratorState::LaneAcquired { .. } => {
                let Some(lane_acquired) = self.lane_acquired.take() else {
                    let outcome = JobOutcome::skipped(
                        "orchestrator missing lane context while in LaneAcquired state",
                    );
                    self.complete_with_outcome(ctx.candidate.spec.job_id.clone(), outcome.clone());
                    return StepOutcome::Done(outcome);
                };
                match Self::run_lane_prepared_phase(ctx, lane_acquired) {
                    LanePreparedOutcome::Done(outcome) => {
                        self.complete_with_outcome(
                            ctx.candidate.spec.job_id.clone(),
                            outcome.clone(),
                        );
                    ...
                    LanePreparedOutcome::Advanced(lease_persisted) => {
                        let lease_persisted = *lease_persisted;
                        let lane_id = lease_persisted.acquired.lane_id.clone();
                        self.lease_persisted = Some(lease_persisted);
                        self.transition(OrchestratorState::LeasePersisted {
                            job_id: ctx.candidate.spec.job_id.clone(),
                            lane_id,
                        });
                        StepOutcome::Advanced
            ...
            OrchestratorState::LeasePersisted { job_id, lane_id } => {
                self.transition(OrchestratorState::Executing { job_id, lane_id });
                StepOutcome::Advanced
            },
            OrchestratorState::Executing { job_id, lane_id } => {
                let Some(lease_persisted) = self.lease_persisted.take() else {
                    let outcome = JobOutcome::skipped(
                        "orchestrator missing lease context while in Executing state",
                    );
                    self.complete_with_outcome(ctx.candidate.spec.job_id.clone(), outcome.clone());
                    return StepOutcome::Done(outcome);
                };
                let outcome = Self::run_execute_phase(ctx, lease_persisted);
                self.staged_outcome = Some(outcome);
                self.transition(OrchestratorState::Committing { job_id, lane_id });
                StepOutcome::Advanced
            },
            OrchestratorState::Committing { .. } => {
                let outcome = self.staged_outcome.take().unwrap_or_else(|| {
                    JobOutcome::skipped("orchestrator reached committing without staged outcome")
                });
                self.complete_with_outcome(ctx.candidate.spec.job_id.clone(), outcome.clone());
                StepOutcome::Done(outcome)
            },
    ...
    fn run_idle_phase(ctx: &mut OrchestratorContext<'_>) -> IdlePhaseOutcome {
        let candidate = ctx.candidate;
        let queue_root = ctx.queue_root;
        let fac_root = ctx.fac_root;
        let completed_gates_cache = &mut *ctx.completed_gates_cache;
        let verifying_key = ctx.verifying_key;
        let scheduler = ctx.scheduler;
        let lane = ctx.lane;
        let broker = &mut *ctx.broker;
        let policy_hash = ctx.policy_hash;
        let policy_digest = ctx.policy_digest;
        let policy = ctx.policy;
        let job_spec_policy = ctx.job_spec_policy;
        let budget_cas = ctx.budget_cas;
        let canonicalizer_tuple_digest = ctx.canonicalizer_tuple_digest;
        let boundary_id = ctx.boundary_id;
        let cost_model = ctx.cost_model;
        let toolchain_fingerprint = ctx.toolchain_fingerprint;
        ...
        let file_name = match path.file_name().and_then(|n| n.to_str()) {
            Some(n) => n.to_string(),
            None => {
                return IdlePhaseOutcome::Done(JobOutcome::skipped("invalid filename"));
            },
        };
        ...
        // NIT-2: Compute sandbox hardening hash once at the top of the idle phase
        ...
        if let Err(e) = validation_result {
            ...
            if is_digest_error {
                ...
                if let Err(commit_err) = commit_claimed_job_via_pipeline(
                    ...
                    return IdlePhaseOutcome::Done(JobOutcome::skipped_pipeline_commit(format!(
                        "pipeline commit failed for quarantined job (digest mismatch): {commit_err}"
                    )));
                }
                return IdlePhaseOutcome::Done(JobOutcome::Quarantined { reason });
            }
            ...
            let reason_code = match e {
                ...
                | JobSpecError::InvalidControlLaneRepoId { .. } => {
        ...
        if is_stop_revoke {
            ...
            if let Err(e) = consume_authority(queue_root, &spec.job_id, &spec.job_spec_digest) {
                ...
                if let Err(commit_err) = commit_claimed_job_via_pipeline_with_guard(
                    ...
                    return IdlePhaseOutcome::Done(handle_pipeline_commit_failure(
                        &commit_err,
            ...
            let sr_admission_trace =
                apm2_core::economics::queue_admission::StopRevokeAdmissionTrace {
                    verdict: "allow".to_string(),
                    reservation_used,
            ...
            return IdlePhaseOutcome::Done(handle_stop_revoke(
                spec,
                &claimed_path,
                &claimed_file_name,
                &claimed_lock_guard,
        ...
        if !defects.is_empty() {
            let reason = format!(
                ...
                defects
                    .iter()
                    .map(|d| strip_json_string_quotes(&serialize_to_json_string(
                        &d.violation_class
                    )))
                    .collect::<Vec<_>>()
                    .join(", ")
        ...
        let budget_trace = {
            ...
            if budget_decision.verdict != BudgetAdmissionVerdict::Allow {
                ...
                return IdlePhaseOutcome::Done(JobOutcome::Denied {
        ...
        //
        // Keep this lock file alive for the entire remainder of the orchestration
    ...
    #[allow(clippy::too_many_arguments, clippy::too_many_lines)]
    fn run_claimed_acquire_phase(
        ctx: &OrchestratorContext<'_>,
        claimed: ClaimedContext,
    ) -> ClaimedAcquireOutcome {
        let candidate = ctx.candidate;
        let queue_root = ctx.queue_root;
        let fac_root = ctx.fac_root;
        let policy_hash = ctx.policy_hash;
        let canonicalizer_tuple_digest = ctx.canonicalizer_tuple_digest;
        let heartbeat_cycle_count = ctx.heartbeat_cycle_count;
        let heartbeat_jobs_completed = ctx.heartbeat_jobs_completed;
        let heartbeat_jobs_denied = ctx.heartbeat_jobs_denied;
        let heartbeat_jobs_quarantined = ctx.heartbeat_jobs_quarantined;
        let toolchain_fingerprint = ctx.toolchain_fingerprint;
        ...
        if spec.kind == "gates" {
            ...
            return ClaimedAcquireOutcome::Done(gates_outcome);
        }
        ...
        let lane_mgr = match LaneManager::new(fac_root.to_path_buf()) {
            ...
            Err(e) => {
                ...
                return ClaimedAcquireOutcome::Done(JobOutcome::skipped(format!(
                    "lane manager init failed: {e}"
                )));
            },
        };
        ...
        let Some((lane_guard, acquired_lane_id)) = acquire_worker_lane(&lane_mgr, &lane_ids) else {
            ...
            return ClaimedAcquireOutcome::Done(JobOutcome::skipped_no_lane(
                "no lane available, returning to pending",
            ));
        };
        ClaimedAcquireOutcome::Advanced(Box::new(LaneAcquiredContext {
            claimed: ClaimedContext {
                claimed_path,
                claimed_file_name,
                claimed_lock_guard,
    ...
    #[allow(clippy::too_many_arguments, clippy::too_many_lines)]
    fn run_lane_prepared_phase(
        ctx: &OrchestratorContext<'_>,
        lane_acquired_ctx: LaneAcquiredContext,
    ) -> LanePreparedOutcome {
        let candidate = ctx.candidate;
        let queue_root = ctx.queue_root;
        let fac_root = ctx.fac_root;
        ...
        let policy = ctx.policy;
        let canonicalizer_tuple_digest = ctx.canonicalizer_tuple_digest;
        let print_unit = ctx.print_unit;
        let toolchain_fingerprint = ctx.toolchain_fingerprint;

        let spec = &candidate.spec;
        let LaneAcquiredContext {
            claimed,
            lane_mgr,
            lane_guard,
            lane_id: acquired_lane_id,
        ...
        if let Err(error) = run_preflight(
            ...
            if let Err(commit_err) = commit_claimed_job_via_pipeline_with_guard(
                ...
                return LanePreparedOutcome::Done(handle_pipeline_commit_failure(
                    &commit_err,
            ...
            return LanePreparedOutcome::Done(JobOutcome::Denied { reason });
        }
        ...
        LanePreparedOutcome::Advanced(Box::new(LeasePersistedContext {
            ...
            dir: lane_dir,
            profile_hash: lane_profile_hash,
            systemd_properties: lane_systemd_properties,
        }))
    }
    ...
    fn run_execute_phase(
        ctx: &OrchestratorContext<'_>,
        lease_persisted_ctx: LeasePersistedContext,
    ) -> JobOutcome {
        let candidate = ctx.candidate;
        let queue_root = ctx.queue_root;
        let fac_root = ctx.fac_root;
        ...
        let spec = &candidate.spec;
        let LeasePersistedContext {
            acquired: lane_acquired,
            workspace: persisted_lane_workspace,
            dir: persisted_lane_dir,
            profile_hash: lane_profile_hash,
            systemd_properties: lane_systemd_properties,
        } = lease_persisted_ctx;
        let LaneAcquiredContext {
        ...
        let lane_dir = persisted_lane_dir;
        ...
        if spec.source.kind == "patch_injection" {
            ...
            let patch_bytes: Vec<u8> = match bytes_backend {
                ...
                Some("apm2_cas") => {
                    ...
                    let Some(hash_bytes) =
                        apm2_core::fac::job_spec::parse_b3_256_digest(digest_str)
                    else {
                ...
                Err(apm2_core::fac::RepoMirrorError::PatchHardeningDenied { reason, receipt }) => {
                    ...
                    if let Err(e) = std::fs::create_dir_all(&patch_receipts_dir) {
                        ...
                        if let Err(e) = std::fs::write(&receipt_file, &body) {
                            eprintln!(
                                "worker: WARNING: failed to persist patch denial receipt: {e}"
                            );
                        }
                    }
        ...
        let containment_trace = match apm2_core::fac::containment::verify_containment(
            ...
            Ok(verdict) => {
                ...
                let server_containment = if policy.sccache_enabled {
                    let sc =
                        apm2_core::fac::containment::execute_sccache_server_containment_protocol(
                            std::process::id(),
        ...
        if let Err(commit_err) = commit_claimed_job_via_pipeline_with_guard(
            ...
            if owns_sccache_server(containment_trace.as_ref()) && !sccache_server_env.is_empty() {
                let stopped = apm2_core::fac::stop_sccache_server(&sccache_server_env);
                eprintln!(
                    "worker: sccache server stop (pipeline commit failure): stopped={stopped}"
                );
            }
        ...
        if let Err(cleanup_err) = execute_lane_cleanup(
            ...
            // Lane is already marked corrupt by execute_lane_cleanup on
            // failure. The job outcome remains Completed —
            // infrastructure failures do not retroactively negate
