...
- TCK-00540: Legacy gate cache v2 reuse policy — default deny unless bound to RFC-0028/0029 receipts; dual-format signature verification for tamper resistance.
  - `gate_cache.rs`: `CachedGateResult` gains `rfc0028_receipt_bound`, `rfc0029_receipt_bound`, and `legacy_cache_override` fields (all `bool`, `#[serde(default)]` to `false` for fail-closed deserialization of legacy entries). `set_with_attestation()` writes entries with **fail-closed defaults** (`rfc0028_receipt_bound=false, rfc0029_receipt_bound=false, legacy_cache_override=false`). Flags are only promoted to `true` after a verified receipt lookup via `bind_receipt_evidence()` or `try_bind_receipt_from_store()`. `rebind_gate_cache_after_receipt()` is the public post-receipt binding entry point: loads cache, binds receipt evidence, re-signs, and re-saves. `mark_legacy_override()` method downgrades an entry to `rfc0028_receipt_bound=false, rfc0029_receipt_bound=false, legacy_cache_override=true` when reuse was via `--allow-legacy-cache` (preserves override audit trail so future default-mode runs can still detect and deny). **CRITICAL:** `bind_receipt_evidence()`, `mark_legacy_override()`, and `try_bind_receipt_from_store()` must all be called **before** `sign()`/`sign_all()` because policy flags are now included in the signed content.
  - `fac_worker.rs`: After the worker commits a successful gates job receipt via `commit_claimed_job_via_pipeline`, `fac_review_api::rebind_gate_cache_after_receipt()` is called to promote the gate cache's fail-closed defaults to `rfc0028=true, rfc0029=true` based on the newly-created receipt's RFC-0028/0029 bindings. This closes the chicken-and-egg gap where the gate cache is written before the receipt exists.
  - **Dual-format `canonical_bytes` (BLOCKER fix):** `CanonicalBytesFormat` enum (`WithPolicyFlags`, `WithoutPolicyFlags`) controls whether policy flags are included in the signed canonical bytes. New entries (`WithPolicyFlags`) append 3 bytes for `rfc0028_receipt_bound`, `rfc0029_receipt_bound`, `legacy_cache_override` — cryptographically binding them to the Ed25519 signature. Legacy entries (`WithoutPolicyFlags`) omit these bytes for backward compatibility with pre-BLOCKER-fix cache entries.
  - **Dual-format `verify()`:** Returns `Result<CanonicalBytesFormat, String>`. Attempts verification with `WithPolicyFlags` first (new entries), falls back to `WithoutPolicyFlags` (legacy entries), hard-denies if neither verifies. The returned format tells `check_reuse()` how to interpret YAML policy flags.
  - **Tamper-resistant `check_reuse()`:** Uses the verified format to determine effective policy flag values. `WithPolicyFlags` (new entries): trusts YAML values because they are cryptographically signed — tampering invalidates the signature. `WithoutPolicyFlags` (legacy entries): forces effective flags to `false` regardless of YAML values — an attacker who flips `rfc0028_receipt_bound=true` in a legacy entry's YAML cannot bypass the policy gate because the effective value is always `false` for legacy-format entries. Entries without both effective `rfc0028_receipt_bound` and `rfc0029_receipt_bound` are denied in default mode (fail-closed); `--allow-legacy-cache` flag permits unsafe override returning `legacy_cache_override_unsafe` reason.
  - `evidence.rs`: All 5 cache-hit rewrite paths (Phase 1 through Phase 4) call `gate_cache.mark_legacy_override(gate_name)` after `set_with_attestation()` when `reuse.reason == "legacy_cache_override_unsafe"`, ensuring one unsafe run does not silently convert an unbound legacy cache entry into a trusted entry.
  - `fac.rs`: `--allow-legacy-cache` CLI flag added to `apm2 fac gates`.
