...
- TCK-00627: Warm-path performance SLO instrumentation and verification.
  - `gates.rs` (S1): Added `total_duration_ms`, `total_gate_count`, `cache_hit_count`, `cache_miss_count`, `is_warm_run`, `slo_violation` fields to `GatesSummary`. `run_summary_event()` emits all new fields in the NDJSON run_summary event. `total_duration_ms` computed as `prep_duration_ms.saturating_add(execute_duration_ms)` in `run_gates_inner_detailed()`. `total_gate_count` set from `gate_results.len()` (evidence gate count, excluding merge gate).
  - `gates.rs` (S2): Added `compute_cache_counts()` (pure function, counts hits/misses from `EvidenceGateResult.cache_decision`) and `compute_warm_path_slo()` (pure function, derives `is_warm_run` and `slo_violation` from total gate count, hits, and prep duration). SLO threshold: `WARM_PATH_PREP_THRESHOLD_MS = 500`. `is_warm_run = true` iff `cache_hit_count == total_gate_count && total_gate_count > 0 && prep_duration_ms <= 500`. Uses `total_gate_count` (not `cache_miss_count`) to prevent uncacheable gates (those with `cache_decision: None`) from being silently ignored. SLO violation emits `eprintln!` warning only, never affects exit code.
  - `gates.rs` (S3): Added `ci_benchmark_warm_path_slo_two_consecutive_runs` integration test that invokes `run_gates_inner` for run1 (cold path) and runs the full pipeline including `run_prep_phase` + evidence gates with cache reuse for run2 (warm path) against a hermetic git workspace with fake cargo. Run2 `prep_duration_ms` comes from actual pipeline execution (timed via `Instant`), not hardcoded values. SLO assertion is strictly `run2_total_ms <= run1_total_ms * 20 / 100` with no ad-hoc floor. No external services required.
  - `gates.rs` (S4): Added 11 unit regression tests for `compute_warm_path_slo()` and `compute_cache_counts()`: boundary cases (prep == 500, prep == 501), zero-gate edge case, cache miss scenarios, uncacheable gate prevention, SLO violation string content, exit-code independence, `EvidenceGateResult` integration, and `run_summary_event` field presence verification.
