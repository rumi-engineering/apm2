...
- TCK-00540: Legacy gate cache v2 reuse policy — default deny unless bound to RFC-0028/0029 receipts.
  - `gate_cache.rs`: `CachedGateResult` gains `rfc0028_receipt_bound`, `rfc0029_receipt_bound`, and `legacy_cache_override` fields (all `bool`, `#[serde(default)]` to `false` for fail-closed deserialization of legacy entries). `set_with_attestation()` writes entries with `rfc0028_receipt_bound=true, rfc0029_receipt_bound=true, legacy_cache_override=false` by default (new pipeline runs carry receipt bindings). `mark_legacy_override()` method downgrades an entry to `rfc0028_receipt_bound=false, rfc0029_receipt_bound=false, legacy_cache_override=true` when reuse was via `--allow-legacy-cache` (preserves override audit trail so future default-mode runs can still detect and deny). `check_reuse()` gate: entries without both `rfc0028_receipt_bound` and `rfc0029_receipt_bound` are denied in default mode (fail-closed); `--allow-legacy-cache` flag permits unsafe override returning `legacy_cache_override_unsafe` reason. Policy flags (`rfc0028_receipt_bound`, `rfc0029_receipt_bound`, `legacy_cache_override`) are intentionally EXCLUDED from `canonical_bytes` — they are enforcement policy metadata layered on top after signature verification, not part of the signed gate result. This preserves backward compatibility with pre-TCK-00540 cache entries and eliminates sign/verify mismatch after `mark_legacy_override`.
