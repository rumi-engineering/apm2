...
- TCK-00575: Per-lane HOME/TMPDIR/XDG env isolation + symlink safety + concurrency.
  ...
  - `evidence.rs`: `build_pipeline_test_command()` and `build_gate_policy_env()` accept a `lane_dir` parameter from the actually-locked lane (returned by `allocate_lane_job_logs_dir`) and call `ensure_lane_env_dirs()` + `apply_lane_env_overrides()` using that lane directory, maintaining lock/env coupling. `allocate_lane_job_logs_dir()` returns an `AllocatedLane` struct containing `logs_dir`, `lane_dir`, and `_lane_guard` so callers use the correct lane for env overrides. Every FAC gate phase (fmt, clippy, doc, test, script) runs with deterministic lane-local `HOME`/`TMPDIR`/`XDG_CACHE_HOME`/`XDG_CONFIG_HOME` values scoped to the locked lane.
  - `policy.rs`: `ensure_lane_env_dirs()` uses atomic creation (mkdir + handle `AlreadyExists`) instead of `exists()` check, eliminating TOCTOU. `verify_dir_permissions()` is the shared public helper for directory permission verification (symlink rejection, ownership check, mode 0o700), used by both `verify_lane_env_dir_permissions()` and `verify_cargo_home_permissions()`. UID comparison uses `subtle::ConstantTimeEq`.
