...
- TCK-00575: Per-lane HOME/TMPDIR/XDG env isolation.
  - `gates.rs`: `compute_nextest_test_environment()` now calls `ensure_lane_env_dirs()` to create per-lane env isolation directories (`home/`, `tmp/`, `xdg_cache/`, `xdg_config/`) under the synthetic `lane-00` directory, then calls `apply_lane_env_overrides()` to set `HOME`, `TMPDIR`, `XDG_CACHE_HOME`, and `XDG_CONFIG_HOME` to per-lane paths. This prevents CLI gate execution from writing into ambient user directories (`~/.cache`, `~/.cargo`, `~/.config`).
